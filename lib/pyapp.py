...
class Pyapp(Pyse):
    ...
    def is_exist_element_wait(self, css, secs):
        flag = False
        if "=>" not in css:
            raise NameError("Positioning syntax errors, lack of '=>'.")
        ...
        if by == "id":
            try:
                ...
                logger.error("!!超时!! "+str(secs)+"秒后还没找到元素 ")
            except NoSuchElementException:
                logger.error("!!没有此元素!!"+str(secs)+"秒后还没找到元素 ")
            except Exception:
                logger.error("!!未知异常!!" + str(secs) + "秒后还没找到元素 ")
        elif by == "name":
            try:
                WebDriverWait(self.d, secs, 1).until(EC.presence_of_element_located((MobileBy.NAME, value)))
                flag = True
            ...
            try:
                ...
                logger.debug("!!超时!! "+str(secs)+"秒后还没找到元素 ")
            except NoSuchElementException:
                logger.debug("!!没有此元素!!"+str(secs)+"秒后还没找到元素 ")
            except Exception:
                logger.debug("!!未知异常!!" + str(secs) + "秒后还没找到元素 ")
        elif by == 'ios':
            try:
                WebDriverWait(self.d, secs, 1).until(
                    EC.presence_of_element_located((MobileBy.IOS_PREDICATE, value)))
    ...
    def wait_and_save_exception(self, css, name):
        try:
            ...
            # import threading
            self.get_windows_img(APPPICTUREPATH + name + '.png')
            return False